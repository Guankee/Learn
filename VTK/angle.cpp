...
int main() {
  ...
  planeSource->SetOrigin(p1(0), p1(1), p1(2));
  ...
  Eigen::Vector3d oldPoint = p3 + 1 * n;
  double startPoint[3] = {p1(0), p1(1), p1(2)};  // 箭头根部
  ...
  vtkSmartPointer<vtkActor> lineactor = creatLine(p3, oldPoint);
  Eigen::Vector3d vec;
  rotateNormal(n,p1,p3,-45,vec);
  Eigen::Vector3d newPoint = p1 + 1.414 * vec;
  ...
  vtkSmartPointer<vtkActor> newlineactor = creatLine(p1, newPoint);
