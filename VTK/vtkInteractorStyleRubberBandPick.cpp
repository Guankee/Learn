...
//         vtkSmartPointer<vtkIdList> edgeNeighborCells = vtkSmartPointer<vtkIdList>::New();
//         polyData->GetCellEdgeNeighbors(cellId, ptId1, ptId2, edgeNeighborCells);
...
std::vector<vtkIdType> GetCellNeighbors(vtkSmartPointer<vtkPolyData> poly, vtkIdType cellId) {
...
bool IsMeshBoundary(vtkSmartPointer<vtkPolyData> poly, vtkIdType cellId, int maxIter, std::unordered_set<vtkIdType>& set) {
...
bool checkPointInsidePolyData(vtkSmartPointer<vtkPolyData> poly, std::vector<double> points) {
	double point[3];
	...
	if (isBoundary) {
		std::cout << "------------The initial cell or its neighbors are on the boundary.------------" << std::endl;
	}
	else {
		std::cout << "The initial cell and its neighbors are not on the boundary." << std::endl;
	}


	// // Highlight the visited cells
	vtkSmartPointer<vtkPolyData> highlightPolyData = vtkSmartPointer<vtkPolyData>::New();
  ...
  vtkSmartPointer<vtkPolyData> PolyData3 = vtkSmartPointer<vtkPolyData>::New();
	...
	vtkSmartPointer<vtkCellArray> highlightCells = vtkSmartPointer<vtkCellArray>::New();
  vtkSmartPointer<vtkCellArray> cell1 = vtkSmartPointer<vtkCellArray>::New();
	...
	//renderer->GetActiveCamera()->SetPosition(pointArray[0] + 1, pointArray[1] + 1, pointArray[2] + 1);
	...
	//		std::cerr << "Edge " << ptId1 << "-" << ptId2 << "------------- has no neighbor cells-----------." << std::endl;
	//		return false;
	//	}
	//	else {
...
int main(int, char* []) {
	...
	// std::vector<double> points{ 0.71438033083643981, 0.00081929035976531506,0.056970278379591427 };
	std::vector<double> points{ 0.70438033083643981, 0.00881929035976531506,0.056970278379591427 };
