...
#ifndef vtkBoxWidgetNT_h
#define vtkBoxWidgetNT_h
...
class  vtkBoxWidgetNT : public vtk3DWidget
{
public:
  /**
   * Instantiate the object.
   */
  static vtkBoxWidgetNT* New();

  vtkTypeMacro(vtkBoxWidgetNT, vtk3DWidget);
  ...
  //PlaceWidget()中会调用一次PlaceWidget(double bounds[6])
  ...
  void SetScalingRangeValue(double v);
  /**
  ...
  vtkSetMacro(ScalingRangeEnabled, vtkTypeBool);
  vtkGetMacro(ScalingRangeEnabled, vtkTypeBool);
  vtkBooleanMacro(ScalingRangeEnabled, vtkTypeBool);
...
protected:
  ...
  //控制限制拉伸范围 总体mesh大小的多少倍 
  double ScalingRangeBounds[6];
  vtkTypeBool ScalingRangeEnabled;
  double ScalingRange;

private:
  vtkBoxWidgetNT(const vtkBoxWidgetNT&) = delete;
  void operator=(const vtkBoxWidgetNT&) = delete;
};
