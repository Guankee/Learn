...
vtkStandardNewMacro(vtkBoxWidgetNT);

vtkBoxWidgetNT::vtkBoxWidgetNT()
{
  this->State = vtkBoxWidgetNT::Start;
  this->EventCallbackCommand->SetCallback(vtkBoxWidgetNT::ProcessEvents);
...
*/
  ...
  //0.001
  this->HandlePicker->SetTolerance(1e-7);
  for (i = 0; i < 7; i++)
  {
  ...
  this->HexPicker->SetTolerance(1e-7);
...
void vtkBoxWidgetNT::SetEnabled(int enabling)
{
  if (!this->Interactor)
  {
...
void vtkBoxWidgetNT::ProcessEvents(
  vtkObject* vtkNotUsed(object), unsigned long event, void* clientdata, void* vtkNotUsed(calldata))
{
  vtkBoxWidgetNT* self = reinterpret_cast<vtkBoxWidgetNT*>(clientdata);
...
void vtkBoxWidgetNT::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkBoxWidgetNT::PositionHandles()
{
  ...
  */
  ...
  VTK_AVERAGE(p0, p7, x);
  this->Points->SetPoint(8, x); //左
  VTK_AVERAGE(p1, p6, x);
  this->Points->SetPoint(9, x); //右
  VTK_AVERAGE(p0, p5, x);
  this->Points->SetPoint(10, x);//下
  VTK_AVERAGE(p2, p7, x);
  this->Points->SetPoint(11, x);//上
  VTK_AVERAGE(p1, p3, x);
  this->Points->SetPoint(12, x);//前
  VTK_AVERAGE(p5, p7, x);
  this->Points->SetPoint(13, x);//后
  VTK_AVERAGE(p0, p6, x);
  this->Points->SetPoint(14, x);//中间
...
void vtkBoxWidgetNT::HandlesOff()
{
  for (int i = 0; i < 7; i++)
  {
...
void vtkBoxWidgetNT::SizeHandles()
{
...
int vtkBoxWidgetNT::HighlightHandle(vtkProp* prop)
{
...
void vtkBoxWidgetNT::HighlightFace(int cellId)
{
  if (cellId >= 0)
  {
    vtkIdType npts;
...
void vtkBoxWidgetNT::HighlightOutline(int highlight)
{
  if (highlight)
  {
...
void vtkBoxWidgetNT::OnLeftButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];

  //std::cout << "-------------vtkBoxWidgetNT OnLeftButtonDown: " << X << "," << Y << " ---------" << std::endl;
  // Okay, we can process this. Try to pick handles first;
  // if no handles picked, then pick the bounding box.
  if (!this->CurrentRenderer || !this->CurrentRenderer->IsInViewport(X, Y))
  {
	  this->State = vtkBoxWidgetNT::Outside;
	  return;
  }
  ...
  if (path != nullptr)
  {
    this->State = vtkBoxWidgetNT::Moving;
...
void vtkBoxWidgetNT::OnLeftButtonUp()
{
  if (this->State == vtkBoxWidgetNT::Outside || this->State == vtkBoxWidgetNT::Start)
  {
    return;
  }
...
void vtkBoxWidgetNT::OnMiddleButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
...
void vtkBoxWidgetNT::OnMiddleButtonUp()
{
  if (this->State == vtkBoxWidgetNT::Outside || this->State == vtkBoxWidgetNT::Start)
  {
    return;
...
void vtkBoxWidgetNT::OnRightButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if (path != nullptr)
  {
    this->State = vtkBoxWidgetNT::Scaling;
    this->HighlightOutline(1);
    this->HandlePicker->GetPickPosition(this->LastPickPosition);
    this->ValidPick = 1;
  }
...
void vtkBoxWidgetNT::OnRightButtonUp()
{
  if (this->State == vtkBoxWidgetNT::Outside)
  {
    return;
  }
...
void vtkBoxWidgetNT::OnMouseMove()
{
  ...
  // Process the motion
  if (this->State == vtkBoxWidgetNT::Moving)
  {
    // Okay to process
    if (this->CurrentHandle)
    {
      ...
      {
        camera->GetViewPlaneNormal(vpn);//视平面法线
  ...
  else if (this->ScalingEnabled && this->State == vtkBoxWidgetNT::Scaling)
  {
...
void vtkBoxWidgetNT::MoveFace(double* p1, double* p2, double* dir, double* x1, double* x2, double* x3, double* x4,
    double* x5, int index, int flag, double max)
{
    ...
    v[index]=f* v2[index];
    if (flag){
        if (x5[index]+v[index]> x5[index]&& x5[index] + v[index]> max)
        {
            return;
        }
    }
...
void vtkBoxWidgetNT::GetDirection(
...
void vtkBoxWidgetNT::MovePlusXFace(double* p1, double* p2)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  ...
  if (ScalingRangeEnabled)
  {
      this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1, 0,1, ScalingRangeBounds[1]);
  }
  else
  {
...
void vtkBoxWidgetNT::MoveMinusXFace(double* p1, double* p2)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  ...
  if (ScalingRangeEnabled)
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1, 0, 0, ScalingRangeBounds[0]);
  }
  else
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1);
...
void vtkBoxWidgetNT::MovePlusYFace(double* p1, double* p2)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  ...
  if (ScalingRangeEnabled)
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1, 1, 1, ScalingRangeBounds[3]);
  }
  else
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1);
...
void vtkBoxWidgetNT::MoveMinusYFace(double* p1, double* p2)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  ...
  if (ScalingRangeEnabled)
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1, 1, 0, ScalingRangeBounds[2]);
  }
  else
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1);
...
void vtkBoxWidgetNT::MovePlusZFace(double* p1, double* p2)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  ...
  if (ScalingRangeEnabled)
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1, 2, 1, ScalingRangeBounds[5]);
  }
  else
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1);
...
void vtkBoxWidgetNT::MoveMinusZFace(double* p1, double* p2)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  ...
  if (ScalingRangeEnabled)
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1, 2, 0, ScalingRangeBounds[4]);
  }
  else
  {
	  this->MoveFace(p1, p2, dir, x1, x2, x3, x4, h1);
...
void vtkBoxWidgetNT::Translate(double* p1, double* p2)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  double v[3];
...
void vtkBoxWidgetNT::Scale(double* vtkNotUsed(p1), double* vtkNotUsed(p2), int vtkNotUsed(X), int Y)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
...
void vtkBoxWidgetNT::ComputeNormals()
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
...
void vtkBoxWidgetNT::GetPlanes(vtkPlanes* planes)
{
  if (!planes)
  {
    return;
...
void vtkBoxWidgetNT::Rotate(int X, int Y, double* p1, double* p2, double* vpn)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  double* center = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(3 * 14);
...
void vtkBoxWidgetNT::CreateDefaultProperties()
{
  // Handle properties
...
void vtkBoxWidgetNT::PlaceWidget(double bds[6])
{
  int i;
  ...
  this->SetScalingRangeValue(1.8);
  this->PositionHandles();
  this->ComputeNormals();
  this->SizeHandles();
}

void vtkBoxWidgetNT::GetTransform(vtkTransform* t)
{
  double* pts = static_cast<vtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  double* p0 = pts;
  double* p1 = pts + 3 * 1;
...
void vtkBoxWidgetNT::SetTransform(vtkTransform* t)
{
  if (!t)
  {
...
void vtkBoxWidgetNT::SetScalingRangeValue(double v)
{

	ScalingRange = v;
	double scale = (ScalingRange - 1.0) / 2.0;
	for (int i = 0; i < 6; i=i+2)
	{
		double L = (InitialBounds[i + 1] - InitialBounds[i]) * scale;
		ScalingRangeBounds[i] = InitialBounds[i] - L * scale;
		ScalingRangeBounds[i + 1] = InitialBounds[i + 1] + L * scale;
	}

}

void vtkBoxWidgetNT::GetPolyData(vtkPolyData* pd)
{
  pd->SetPoints(this->HexPolyData->GetPoints());
  pd->SetPolys(this->HexPolyData->GetPolys());
}

void vtkBoxWidgetNT::SetOutlineFaceWires(int newValue)
{
...
void vtkBoxWidgetNT::SetOutlineCursorWires(int newValue)
{
...
void vtkBoxWidgetNT::GenerateOutline()
{
...
//------------------------------------------------------------------------------
void vtkBoxWidgetNT::RegisterPickers()
{
